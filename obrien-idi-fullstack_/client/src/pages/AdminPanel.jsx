...
export default function AdminPanel() {
  ...
  useEffect(() => {
    setPendingRoleSelections(prev => {
      ...
      pending.forEach(user => {
        if (!next[user.id]) {
          next[user.id] = user.role === 'Pending Approval' ? user.desiredRole : user.role;
        }
      });
  ...
  const handleToggleActive = async user => {
    ...
    showFeedback(
      ...
      result.ok ? `${user.name} has been ${user.isActive ? 'deactivated' : 'reactivated'}.` : result.message,
    );
  };
        ...
        <main className="p-6">
          <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Approve requests, assign roles, and manage all user accounts.</p>
            </div>
          ...
          <section className="mt-6 grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
            <div className="card p-5">
              ...
              <div className="mt-5 space-y-4">
                {pending.length === 0 ? (
                  <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600">No pending requests.</div>
                ) : pending.map(user => (
                  <div key={user.id} className="rounded-xl border border-slate-200 p-4">
                    ...
                    <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                      <select
                        value={pendingRoleSelections[user.id] || user.desiredRole || 'Engineer / Operator'}
                      ...
                      <button
                        ...
                        className="whitespace-nowrap rounded-xl bg-brand-600 px-4 py-2 text-xs font-semibold text-white hover:bg-brand-700"
                      >
                        Approve
                      </button>
                    </div>
                        ...
                        <tr key={user.id}>
                          ...
                          <td className="px-4 py-3">
                            <select
                              ...
                              className="input min-w-[160px] py-2 text-xs disabled:cursor-not-allowed disabled:bg-slate-100"
                            >
                              {roleOptions.map(r => <option key={r}>{r}</option>)}
                            </select>
                          </td>
                          <td className="px-4 py-3">
                            <span className={`rounded-full px-2 py-1 text-xs font-semibold ${user.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                              {user.approved ? (user.isActive ? 'Active' : 'Inactive') : 'Pending'}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex flex-wrap gap-2">
                              {!user.approved && (
                                <button onClick={() => handleApprove(user)} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700">
                                  Approve
                                </button>
                              )}
                              <button
                                ...
                                disabled={isProtectedAdmin}
                                title={isProtectedAdmin ? 'Primary demo admin is protected.' : ''}
                                className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
                              >
                                {isProtectedAdmin ? 'Protected' : user.isActive ? 'Deactivate' : 'Reactivate'}
                              </button>
                            </div>
                          </td>
                        </tr>
