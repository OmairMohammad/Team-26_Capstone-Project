...
export default function FleetAssets() {
  ...
  const types = ['All', ...new Set(assessed.map(a => a.assetType))];
  const filtered = assessed.filter(a => {
    const matchSearch = !search || a.assetName.toLowerCase().includes(search.toLowerCase()) || a.assetId.toLowerCase().includes(search.toLowerCase()) || a.site.toLowerCase().includes(search.toLowerCase());
  ...
  const handleExport = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'fleet-assets.csv'; a.click();
        ...
        <main className="p-6">
          <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Detailed condition profile for all {assessed.length} assets. Click any card to expand full data.</p>
            </div>
            ...
            {filtered.map(asset => (
              <div key={asset.assetId} className="card p-5">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  {[['Health Score', `${asset.healthScore}/100`], ['Efficiency', `${asset.efficiencyScore}%`], ['Action', asset.recommendedAction],
                    ['Temperature', `${asset.temperature}°C`], ['Pressure', `${asset.pressure} bar`], ['Vibration', `${asset.vibration} mm/s`]].map(([k, v]) => (
                ...
                {expanded === asset.assetId && (
                  <div className="mt-4 border-t border-slate-100 pt-4 space-y-4">
                    <div className="grid gap-3 sm:grid-cols-3">
                      {[['Install Year', asset.installYear], ['Asset Age', `${asset.assetAge} yrs`], ['Operating Hours', `${asset.operatingHours.toLocaleString()} h`],
                        ['Fault Events', asset.recentFaults], ['Overdue Days', asset.overdueDays], ['Criticality', asset.criticality],
                        ['Bearing Temp', `${asset.bearingTemp}°C`], ['Water Hardness', asset.waterHardness > 0 ? `${asset.waterHardness} mg/L` : 'N/A'],
                        ['Stack Temp', asset.stackTemp > 0 ? `${asset.stackTemp}°C` : 'N/A'],
                        ['Worker Certified', asset.workerCertified ? 'Yes' : 'No'], ['Worker Fatigue', asset.workerFatigue], ['Experience', `${asset.workerExperience} yrs`],
                        ['CO₂ Emissions', asset.co2Emissions > 0 ? `${asset.co2Emissions} kg/h` : 'N/A'], ['NOx Level', asset.noxLevel > 0 ? `${asset.noxLevel} mg/m³` : 'N/A'],
                        ['Emission Compliant', asset.emissionCompliant ? 'Yes ✓' : 'No ✗']].map(([k, v]) => (
                        <div key={k} className="rounded-xl bg-slate-50 p-3 text-sm">
                    ...
                    <div className="rounded-xl border border-slate-200 p-4">
                      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Recommended Maintenance Strategy</p>
                      <p className="mt-1 text-sm font-bold text-slate-900">{asset.strategyRecommendation?.name}</p>
                      <p className="text-sm text-slate-600">{asset.strategyRecommendation?.rationale}</p>
                    </div>
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div className="rounded-xl bg-slate-50 p-3 text-sm">
                        <span className="text-slate-400 text-xs block">Replacement Cost</span>
                        <span className="font-bold text-slate-900">${asset.replacementCost.toLocaleString()}</span>
                      </div>
                      <div className="rounded-xl bg-slate-50 p-3 text-sm">
                        <span className="text-slate-400 text-xs block">Annual Maintenance Cost</span>
                        <span className="font-bold text-slate-900">${asset.annualMaintenanceCost.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                )}
